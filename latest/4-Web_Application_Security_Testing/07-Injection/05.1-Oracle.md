...
This chapter focuses on recognizing and testing PL/SQL Gateway endpoints, including legacy `mod_plsql` and Application Server deployments that remain in production. Apply the same input-validation mindset to ORDS and APEX surfaces, and use the general [SQL Injection](05-SQL_Injection.md) guidance for Oracle-backed applications that do not expose the gateway.

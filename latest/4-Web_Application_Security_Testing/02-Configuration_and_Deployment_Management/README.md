---

layout: col-document
title: WSTG - Latest
tags: WSTG

---

{% include breadcrumb.html %}
<link rel="stylesheet" href="{{ site.baseurl }}/assets/css/wstg-nav.css">
<script src="{{ site.baseurl }}/assets/js/wstg-nav.js" defer></script>
# 4.2 Configuration and Deployment Management

4.2.1 [Network Infrastructure Configuration](01-Network_Infrastructure_Configuration.md)

4.2.2 [Application Platform Configuration](02-Application_Platform_Configuration.md)

4.2.3 [File Extensions Handling for Sensitive Information](03-File_Extensions_Handling_for_Sensitive_Information.md)

4.2.4 [Review Old Backup and Unreferenced Files for Sensitive Information](04-Review_Old_Backup_and_Unreferenced_Files_for_Sensitive_Information.md)

4.2.5 [Enumerate Infrastructure and Application Admin Interfaces](05-Enumerate_Infrastructure_and_Application_Admin_Interfaces.md)

4.2.6 [HTTP Methods](06-HTTP_Methods.md)

4.2.7 [HTTP Strict Transport Security](07-HTTP_Strict_Transport_Security.md)

4.2.8 [RIA Cross Domain Policy](08-RIA_Cross_Domain_Policy.md)

4.2.9 [File Permissions](09-File_Permissions.md)

4.2.10 [Subdomain Takeover](10-Subdomain_Takeover.md)

4.2.11 [Cloud Storage](11-Cloud_Storage.md)

4.2.12 [Content Security Policy](12-Content_Security_Policy.md)

4.2.13 [Path Confusion](13-Path_Confusion.md)

4.2.14 [Other HTTP Security Header Misconfigurations](14-Other_HTTP_Security_Header_Misconfigurations.md)

4.2.15 [Feature Flag Security Bypass](15-Feature_Flag_Security_Bypass.md)

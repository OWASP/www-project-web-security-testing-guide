...
This attack is only possible when the client uses a response type that directly issues an access token to the client. This occurs with the grant types Implicit Flows, Resource Owner Password Credential, and machine-to-machine flows. See [OAuth Weaknesses](05-OAuth_Weaknesses.md) for further description.
...
![A diagram of the access token injection flow](images/05-token-injection.png)\
...
- Avoid deprecated OAuth grant types. See [OAuth Weaknesses](05-OAuth_Weaknesses.md) for further description.

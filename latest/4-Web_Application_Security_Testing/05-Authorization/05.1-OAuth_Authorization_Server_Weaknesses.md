...
Additionally, see the `Common Filter Bypass` section in [Server-Side Request Forgery](../07-Injection/19-Server-Side_Request_Forgery.md) to identity common bypasses for redirect URI validation.
...
### Consent Page Cross-Site Request Forgery
...
Clickjacking is described in [Clickjacking](../11-Client-side/09-Clickjacking.md). When the consent page is prone to clickjacking and the attacker is in possession of the `client_id` (for public clients, or the client secret for confidential clients), the attacker can forge the user's consent and gain access to the requested resource through a rogue client.
...
See [Clickjacking](../11-Client-side/09-Clickjacking.md) for a detailed description of how such an attack can be conducted.
...
- [Client-side URL Redirect](../11-Client-side/04-Client-side_URL_Redirect.md)

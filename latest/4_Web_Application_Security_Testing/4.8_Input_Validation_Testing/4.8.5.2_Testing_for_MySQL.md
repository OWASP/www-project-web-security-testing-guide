...
From now on, we will assume that there is a classic SQL injection vulnerability, which can be triggered by a request similar to the the one described in the Section on [Testing for SQL Injection](4.8.5_Testing_for_SQL_Injection_WSTG-INPVAL-005.md).
...
Out of band injection could be accomplished by using the ['into outfile'](#Write_in_a_File) clause.
